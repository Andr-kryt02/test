...
class PlayerViewController: UIViewController {
	...
	fileprivate var currentSongIx = 0 {
		didSet {
	...
	fileprivate var currentItem : AVPlayerItem? {
		guard let item = library[self.currentSongIx].playerItem else {
			return nil
		}
	...
	*/
	...
	private func updateProgress(time : CMTime) {
		...
		let currentSong = library[currentSongIx]
...
extension PlayerViewController : PlayerControlsDelegate {
	...
	func onRepeat() {
		playerSongListView.setCurrentIndex(index: 2, animated: true)
...
extension PlayerViewController : PlayerSliderProtocol {
	func onValueChanged(progress : Float, timePast : TimeInterval) {
		let tm = CMTime(seconds: timePast, preferredTimescale: 1000)
		beeingSeek = true
		/*self.player.seek(to: tm)*/
	}
}
